...
% System parameters
params.enable_BS2BSchannels=1;      % Enable (1) or disable (0) generation of the channels between basestations                                   % 0: activate instead time domain (TD) channel impulse response generation for non-OFDM systems
...
% Orientation:
params.CDL_5G.bsArrayOrientation = [[0, 0]; [90, 0]]; % azimuth (0 deg is array look direction +x, 90 deg is +y) and elevation (positive points upwards) in deg
...
params.CDL_5G.ueArrayOrientation = [-180, 0]; % azimuth (0 deg is array look direction +x, 90 deg is +y) and elevation (positive points upwards) in deg
